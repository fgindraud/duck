...
template <typename T, std::size_t StorageSize> class SmallUniquePtr {
	 ...
	 * A moved-from pointer is in an unspecified but valid state (null, or moved-from T).
	 */
private:
...
public:
	...
	// Public internal, releases the pointer (even if it is inline storage)
	pointer release_if_pointer_unsafe () noexcept {
		auto tmp = data_;
		data_ = nullptr;
		return tmp;
	}
...
private:
	...
	void move_from_other (SmallUniquePtr<U, OtherStorageSize> && other) noexcept {
		...
		if (other.is_allocated ()) {
			// Simple case, just steal pointer
			data_ = other.release_if_pointer_unsafe ();
		} else {
			// We must call the move constructor to change the storage.
			data_ = create_storage_for_move<SmallUniquePtr<U, OtherStorageSize>> ();
			other->small_unique_ptr_move (data_);
		}
	}
